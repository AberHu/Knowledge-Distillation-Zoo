...
parser = argparse.ArgumentParser(description='logits')
...
parser.add_argument('--lambda_logits', type=float, default=0.1)
...
def main():
	...
	if args.cuda:
		...
		criterionLogits = torch.nn.MSELoss()
	...
	for epoch in range(1, args.epochs+1):
		...
		criterions = {'criterionCls':criterionCls, 'criterionLogits':criterionLogits}
		...
		# save model
		print('saving models......')
		save_name = 'logits_r{}_r{}_{:>03}.ckp'.format(args.t_name[6:], args.s_name[6:], epoch)
...
def train(train_loader, nets, optimizer, criterions, epoch):
	...
	criterionLogits = criterions['criterionLogits']
	...
	for idx, (img, target) in enumerate(train_loader, start=1):
		...
		logits_loss  = criterionLogits(output_s, output_t.detach()) * args.lambda_logits
		loss = cls_loss + logits_loss

		prec1, prec5 = accuracy(output_s, target, topk=(1,5))
		cls_losses.update(cls_loss.item(), img.size(0))
		logits_losses.update(logits_loss.item(), img.size(0))
		...
		if idx % args.print_freq == 0:
			print('Epoch[{0}]:[{1:03}/{2:03}] '
				  ...
				  'Logits:{logits_losses.val:.4f}({logits_losses.avg:.4f})  '
				  'prec@1:{top1.val:.2f}({top1.avg:.2f})  '
				  'prec@5:{top5.val:.2f}({top5.avg:.2f})'.format(
				  epoch, idx, len(train_loader), batch_time=batch_time, data_time=data_time,
				  cls_losses=cls_losses, logits_losses=logits_losses, top1=top1, top5=top5))
...
def test(test_loader, nets, criterions):
	...
	f_l = [cls_losses.avg, logits_losses.avg, top1.avg, top5.avg]
	print('Cls: {:.4f}, Logits: {:.4f}, Prec@1: {:.2f}, Prec@5: {:.2f}'.format(*f_l))
